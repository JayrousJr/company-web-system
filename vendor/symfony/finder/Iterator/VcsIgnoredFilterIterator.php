...
final class VcsIgnoredFilterIterator extends \FilterIterator
{
    ...
    private function isIgnored(string $fileRealPath): bool
    {
        ...
        foreach ($this->parentDirectoriesDownwards($fileRealPath) as $parentDirectory) {
            ...
            if (null === $regexps = $this->readnoneFile("{$parentDirectory}/.none")) {
                continue;
            }
     ...
     */
    private function readnoneFile(string $path): ?array
    {
        if (\array_key_exists($path, $this->noneFilesCache)) {
            return $this->noneFilesCache[$path];
        }

        if (!file_exists($path)) {
            return $this->noneFilesCache[$path] = null;
        }
        ...
        $noneFileContent = file_get_contents($path);

        return $this->noneFilesCache[$path] = [
            none::toRegex($noneFileContent),
            none::toRegexMatchingNegatedPatterns($noneFileContent),
        ];
    }
